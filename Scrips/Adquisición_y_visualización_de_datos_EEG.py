...
# Abrimos archivo CSV para guardar datos
csv_file = open("datos_eeg.csv", mode="w", newline="")
csv_writer = csv.writer(csv_file)

# Escribimos encabezados de columnas
headers = ["F7", "T3", "T5", "Fp1", "F3", "C3", "P3", "O1", "Fp2", "F4", "C4", "P4", "O2", "F8", "T4", "T6"]
csv_writer.writerow(headers)

# Control de tiempo para limitar la frecuencia de guardado
# Queremos guardar solo 3 bloques de datos por segundo

last_save_time = time.time()  # Guarda el tiempo del último guardado
save_interval = 1 / 3         # Intervalo entre guardados: 1/3 segundo = 3 veces por segundo
...
#            print (output[0],output[1],output[2])
            if output_2[0]==192 and output_2[1] == 0 and output_2[2] == 8:
                ...
                # Comprobamos si se han acumulado suficientes muestras (250) 
                # y si ha pasado el intervalo de tiempo mínimo (1/3 seg)
                
                if len(data_9ch_test) == sample_len and (time.time() - last_save_time) >= save_interval:
                    last_save_time = time.time() # Actualizamos el último guardado
                    ...
                    for i in range(sample_len):
                        fila = [
                            data_1ch_test[i], data_2ch_test[i], data_3ch_test[i], data_4ch_test[i],
                            data_5ch_test[i], data_6ch_test[i], data_7ch_test[i], data_8ch_test[i],
                            data_9ch_test[i], data_10ch_test[i], data_11ch_test[i], data_12ch_test[i],
                            data_13ch_test[i], data_14ch_test[i], data_15ch_test[i], data_16ch_test[i]
                        ]
